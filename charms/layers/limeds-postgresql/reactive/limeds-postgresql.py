...
@when_not("database.requested")
def add_database(postgresql_relation):
    postgresql_relation.set_database(DATABASE)
    set_state("database.requested")
...
def re_add_database(postgresql_relation):
    postgresql_relation.set_database(DATABASE)


@when(
    'limeds.available',
    'db.{}.available'.format(DATABASE), )
@when_not(
    'limeds.installable.deployed')
def add_installable(limeds_relation, postgresql_relation):
    deploy_installable(
        limeds_relation.url,
        postgresql_relation.hostname(),
        postgresql_relation.port())
...
@when(
    ...
    'limeds.installable.deployed',
    'config.changed', )
def re_add_installable(limeds_relation, postgresql_relation):
    deploy_installable(
        limeds_relation.url,
        postgresql_relation.hostname(),
        postgresql_relation.port())
...
def deploy_installable(base_url, postgres_hostname, postgres_port):
    ...
    with open("{}/templates/postgresql-config.json".format(charm_dir()), 'r') as conf_file:
