    ...
    for key in facter.keys():
        if key.startswith('ipaddress_'):
            ...
            #
                ...
                netmask = facter["netmask_{}".format(key[10:])]
                network = IPv4Network("{}/{}".format(address, netmask), strict=False)
                internal_networks.append("{} {}".format(network.network_address, network.netmask))
    if not ext_ip:
