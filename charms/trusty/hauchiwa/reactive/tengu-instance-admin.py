...
#     """ Adds hostname and port to config """
#     hookenv.log('reconfiguring tengu-instance-admin rest2jfed relation')
#     hostname = relation_get('hostname')
#     port = relation_get('port')
#     hookenv.log('hostname:{0} port:{1}'.format(hostname, port))
#     if not hostname and port:
#         hookenv.log(\
#              'did not get required relation values. Need: hostname and port')
#         exit(0)
#     with open(GLOBAL_CONF_PATH, 'r') as infile:
#         content = yaml.load(infile)
...
def setup_rest2jfed(rest2jfed):
    with open(GLOBAL_CONF_PATH, 'r') as infile:
        content = yaml.load(infile)
    content['rest2jfed_hostname'] = str(rest2jfed.host())
    content['rest2jfed_port'] = str(rest2jfed.port())
    with open(expanduser(GLOBAL_CONF_PATH), 'w') as config_file:
        config_file.write(yaml.dump(content, default_flow_style=False))
