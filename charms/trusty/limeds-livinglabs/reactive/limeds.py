...
from charmhelpers.core.hookenv import open_port, close_port, config
...
def install_limeds():
    ...
    templating.render(
        ...
        context={
            'description': 'limeds',
            'command': 'cd /opt/limeds/run/ \njava -jar -Dgosh.args=--nointeractive generated/distributions/executable/run.jar',
        }
    )
...
def upgrade_limeds():
    ...
    keypath = '{}/files/id_rsa'.format(hookenv.charm_dir())
    subprocess.check_call(['chmod', 'go-r', keypath])
    subprocess.check_call(['git', 'checkout', 'master'], cwd='/opt/limeds')
    subprocess.check_call([
        # use ssh-agent to use supplied privkey for git ssh connection
        'ssh-agent', 'bash', '-c',
        # remote 'upstream' will point to supplied given repo
    ...
    try:
        subprocess.check_output([
            'gradle', 'jar', 'export'
        ], cwd='/opt/limeds', stderr=subprocess.STDOUT, universal_newlines=True)
