...
# public ip were being dropped. They were being dropped because the server
# sends the answer through a different interface than the one where the request
# came from. Such packets are being dropped as a security measure.
#
# The cause of this strange behavior is that the default gateway the server
# gets from maas is one in the private network, not on the public network. I
# first tried to force responses to be send out from the same interface.
#    see: https://unix.stackexchange.com/questions/4420
# This worked for packets destined for the server itself but the combination of
# port-forwards + SNAT + this fix didn't work.
#
# The current fix is to change the default gateway of a server with a public
# ip. This has the disadvantage that the network topology differs between
# servers that have a public ip and those that don't have one.
#
# To run this script when maas deploys a node, add the following to
# `/etc/maas/preseeds/curtin_userdata` right above "power_state:"
#
...
upcommands = """
    ...
    post-up ip route add default via {gateway} || true
"""
...
# `/etc/network/interfaces` will be that of the installed os but `ifconfig`
# will show the network setup of the installation environment.
#
found = False
...
interfaces = check_output([
    ...
    | grep iface\
    | cut -d " " -f 2\
    | grep -v lo'], shell=True, universal_newlines=True).rstrip()
for interface in interfaces.split('\n'):
    interface = interface.split('@')[0]
    # Note: this awk for some reason does not recognize \s
    # Source of this crazy oneliner:
    #     https://stackoverflow.com/questions/30300170#30300241
    ip = check_output(
        [
            "awk -v par='%s' "
            "'/^iface/ && $2==par {f=1} "
            "/^iface/ && $2!=par {f=0} "
            "f && /^[\\t ]*address/ {print $2; f=0}' "
    ...
    if ip:
        ...
        if GATEWAY in network:
            ...
            with open('/etc/network/interfaces', 'r+') as interfaces_file:
                ...
                interfaces = (
                    interfaces[0:m.end()] +
                    filled_in_upcommands +
                    interfaces[(m.end()+1):])
                print("This is the interfaces file"
                      "after I changed it: \n{}".format(interfaces))
